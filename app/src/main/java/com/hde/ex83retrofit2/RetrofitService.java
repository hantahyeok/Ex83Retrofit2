...
public interface RetrofitService {
    ...
    //1. 단순하게 GET 방식으로 json문자열을 읽어오기
    @GET("04Retrofit/board.json") //접속하는 파일의 경로를 저장
    Call<Item> getJson();

    //2. 경로의  위 1번처럼 고정하지 않고 함수의 파라미터로 전달 받아 지정할 수 있음.
    @GET("{folder}/{fileName}") //접속하는 파일의 경로를 저장
    Call<Item> getJsonToPath(@Path("folder") String path, @Path("fileName")String fileName);
    ...
    //5. GET 방식으로 데이터를 보낼때 Map Collection을 이용하여 한방에 값들 전달해보기 [ @QueryMap ]
    @GET("04Retrofit/getTest.php")
    Call<Item> getMethodTest3(@QueryMap Map<String, String> datas);

    //6. POST 방식으로 데이터들 전달 [@Body] - 객체를 전달하면 자동으로 json문자열로 변환되어 서버에 전달됨
    ...
    //8. GET 방식으로 json array 를 읽어와서 곧바로 ArrayList<Item> 변환
