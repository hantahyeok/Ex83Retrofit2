...
import com.hde.ex83retrofit2.databinding.ActivityMainBinding;
...
public class MainActivity extends AppCompatActivity {
    ...
    // 2. Retrofit - OkHttp를 기반으로 개발한 라이브러러 - sqareup 회사가 보유 [현제 가장 많이 사용됨] - 현제는 version 2
    // 3. Volley - Google 에서 제작..하여서 많은 기대를 받았고 사용되어 왔으나 업데이트 지원정책에 혼선이 발생함.[현재 사용은 조금 애매함]

    // 3개중에서 현업에서 가장 많이 사용되는 Retrofit2 라이브러리 사용.
    ...
    void clickBtn9(){
        ...
        builder.baseUrl("http://tahyeok.dothome.co.kr");
    ...
    void clickBtn7(){
        // POST 방식으로 데이터를 따로따로 보내기 : 마치 GET 방식의 @Query 처럼.
        int no= 2;
        String title= "bbb";
    ...
    void clickBtn6(){
        // POST 방식으로 데이터 보내기 - 보통은 보낼데이터가 객체로 만들어져 있는 경우가 많음. - [Retrofit이 자동으로 객체를 json문자열로 변환하여 POST 함 ]
    ...
    void clickBtn5(){
        // GET 방식으로 보낼 데이터들을 한방에 보내기.. [ Map Collection 이용 ]

        // 서버에 전달할 데이터들을 하나의 Map 으로 묶기.
    ...
    void clickBtn4(){
        ...
        //1) Retrofit 객체생성 - 항상 4줄씩 쓰기가 번거로움. 그래서 별도 클래스에 기능으로 만들기.
        Retrofit retrofit= RetrofitHelper.getRetrofitInstance();

        //2,3) 인터페이스안에 추상메소드 설계 및 객체 생성
        RetrofitService retrofitService= retrofit.create(RetrofitService.class);
        //4) 추상메소드 호출
    ...
    void clickBtn3(){
        ...
        // 2) RetorfitService 인터페이스 설계 [ 원하는 GET 작업 추상메소드 설계 ]
        // getMethodTest(title, message);
        ...
        // 5) 네트워크 작업 시작 밑 응답 받드록 callback 객체 등록
        call.enqueue(new Callback<Item>() {
            @Override
            public void onResponse(Call<Item> call, Response<Item> response) {
                Item item= response.body();
    ...
    void clickBtn2(){
        ...
        //2) RetrofitService 인터페이스 설계 [ 원하는 GET, POST 동닥을 수행하는 추상메소드 설계 ]
    ...
    void clickBtn(){
        ...
        //2. Service 인터페이스 설계 [원하는 GET,POST 동작을 하는 추상메소드 설게]
        // 통신이 이루어졌으면 하는 방법과 경로를 지정해주는 코드

        //3. 2단계작업에서 설계한 RetrofitService 인터페이스를 객체로 생성
        RetrofitService retrofitService= retrofit.create(RetrofitService.class);

        //4. 위에서 만든 서비스객체의 추상메소드를 호출하여 실제 서버작업을 수행하는 코드를 가진
        // Call 이라는 객체를 리턴받기
        Call<Item> call=retrofitService.getJson();

        //5. 위4단계에서 리턴받은 call 객체에게 네트워크 작업을 수행하도록 요청!
        call.enqueue(new Callback<Item>() {
            @Override
            public void onResponse(Call<Item> call, Response<Item> response) {
